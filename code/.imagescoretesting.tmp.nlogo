breed [agnts agnt]
breed [dots dot]
agnts-own [k h payoff imagescore drawn oldk oldh imagescoreothers]
patches-own [freq]
...
to createfigure
  ...
  ask patch 6 3 [ask dots-here [set size 0.0]]
  ask patch 6 4 [ask dots-here [set size 0.02]]
  ask patch 6 5 [ask dots-here [set size 0.022]]
  ask patch 6 6 [ask dots-here [set size 0.008]]
...
to setup
  ...
  create-agnts number [
   set imagescore 0
   set k 6 - random 12
   ifelse h-variable = "none" [set h 0][set h 6 - random 12]
   set payoff 0
   set drawn 0
   set hidden? true
   set imagescoreothers []
   let i 0
   while [i < number]
   [
     set imagescoreothers lput 0 imagescoreothers
     set i i + 1
   ]
  ]
  set cumpayoff 0
...
to go
  ...
  newgeneration
  tick
    set cumpayoff cumpayoff + mean [payoff] of agnts
    set cumcoop cumcoop + nrcoop
  set nrcoop 0
  if ticks > 10000000 [stop]
end

to playgames
  ask agnts [set payoff 0]
  let nrgames 0
  while [nrgames < maxnrgames]
    ;(10 * number)]; maxnrgames]
  [
    let donor one-of agnts
    ask donor [set drawn 1]

    let receiver one-of agnts with [drawn = 0] ; an agent cannot play a game against itself

    if h-variable = "none" [
        ifelse visibility < 1 [
        ifelse ([item ([who] of receiver) imagescoreothers] of receiver >= [k] of donor) [
          ask donor [
            set payoff payoff - c set nrcoop nrcoop + 1 if imagescore < 5 [
              set imagescore imagescore + 1

            ]
            ask agnts [
              if ((self = receiver) or (random-float 1 < visibility)) [
                let whoisdonor 0
                ask donor [set whoisdonor who]
                if item whoisdonor imagescoreothers < 5 [
                  let updated item whoisdonor imagescoreothers + 1
                  set imagescoreothers replace-item whoisdonor imagescoreothers updated
                ]
              ]
            ]
          ]
          ask receiver [set payoff payoff + b]
        ][
          ask donor [if imagescore > (0 - 5) [set imagescore imagescore - 1]
            ask agnts [
               if ((self = receiver) or (random-float 1 < visibility)) [
                 let whoisdonor 0
                 ask donor [set whoisdonor who]
                 if item whoisdonor imagescoreothers > (0 - 5) [
                 let updated item whoisdonor imagescoreothers - 1
                 set imagescoreothers replace-item whoisdonor imagescoreothers updated]
               ]
             ]
      ...
      ifelse [imagescore] of receiver >= [k] of donor [
        ask donor [
          set payoff payoff - c
          set nrcoop nrcoop + 1
          if imagescore < 5 [set imagescore imagescore + 1]
        ]
        ask receiver [set payoff payoff + b]
      ][
      ask donor [if imagescore > (0 - 5) [set imagescore imagescore - 1]]
    ...
    if h-variable = "AND" [
      ifelse visibility < 1 [
        ifelse ([item ([who] of receiver) imagescoreothers] of receiver >= [k] of donor and [imagescore] of donor < [h] of donor) [
          ask donor [
            set payoff payoff - c
            set nrcoop nrcoop + 1
            if imagescore < 5 [
        ...
        ifelse ([imagescore] of receiver >= [k] of donor and [imagescore] of donor < [h] of donor) [
          ask donor [
             set payoff payoff - c
             set nrcoop nrcoop + 1
             if imagescore < 5 [set imagescore imagescore + 1]
    ...
    if h-variable = "OR" [
      ifelse visibility < 1 [
        ifelse ([item ([who] of receiver) imagescoreothers] of receiver >= [k] of donor or [imagescore] of donor < [h] of donor) [
          ask donor [
            set payoff payoff - c
            set nrcoop nrcoop + 1
            if imagescore < 5 [
        ...
        ifelse ([imagescore] of receiver >= [k] of donor or [imagescore] of donor < [h] of donor) [
          ask donor [
            set payoff payoff - c
            set nrcoop nrcoop + 1
            if imagescore < 5 [set imagescore imagescore + 1]
    ...
    ; as stated in Nowak and Sigmund c is added for each interaction to avoid negative payoffs.
   ask donor [set payoff payoff + c]
   ask receiver [set payoff payoff + c]
    ask donor [set drawn 0]
    set nrgames nrgames + 1
  ]
  if ticks > 0 [
    ask agnts [
      ask patch k h [set freq freq + 1]
    ]
  ]

  let maxfreq number * (ticks - 0)
...
to newgeneration
  ...
  ask agnts [
    set sumpayoff sumpayoff + payoff
    set oldk k
    set oldh h
  ]
  ...
  ifelse sumpayoff > 0 [
    ...
    let draw random-float sumpayoff

    let i 0
    let found 0
    let sumpayoff2 0
    while [found = 0 and i < number]
    [
      ifelse draw < sumpayoff2 + [payoff] of agnt i [set found 1]
      [
        set sumpayoff2 sumpayoff2 + [payoff] of agnt i
        set i i + 1
      ]
    ]
  ;  if i = number [set i i - 1]
    ifelse random-float 1 < mutationrate [
      set k 6 - random 12
      ifelse h-variable = "none" [set h 0][set h 6 - random 12]
    ][
      set k [oldk] of agnt i
      set h [oldh] of agnt i
    ]
  ]][
    ask agnts [
      ifelse random-float 1 < mutationrate [
      ...
      ifelse h-variable = "none" [set h 0][set h 6 - random 12]
        ...
        let otheragnt one-of other agnts
        set k [oldk] of otheragnt
        set h [oldh] of otheragnt
      ]
    ]
  ]
  ...
  ask agnts [
   ...
   while [i < number]
   [
     set imagescoreothers replace-item i imagescoreothers 0
     set i i + 1
   ]
  ]
end
...
@#$#@#$#@
This is a Netlogo implementation of the model described in Nowak, M.A. and K. Sigmund (1998) Evolution of indirect reciprocity by image scoring, Nature 393: 573 � 577

The results of Nowak and Sigmund (1998) are replicated with Netlogo version 4.1.1.

The replication is performed by Marco A. Janssen in cooperation with students from the Agent-Based Modeling class AML 591, Arizona State University, October 2010.  
Copyright (C) 2010 M.A. Janssen
